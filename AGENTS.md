...
- **"Block main" ruleset:** `main` is protected by a repository ruleset (requires a PR, no force-push/deletion, and the required status check named **`Lint, Typecheck & Build`**). Codeowner review is **not** required — it was removed so bot PRs (e.g. the changelog sync) can auto-merge unattended; GitHub does not allow the GitHub Actions app as a bypass actor on personal repos, so the requirement was dropped rather than exempted. Never rename that job without updating the ruleset's required checks.
